...
// в скриптах есть updateImports, который позволяет быстро добавить паблик апи в дирректориях (в нашем случае в shared слой), а также заменить по всему проекту путь из publicApi. запускается командой npx ts-node ./scripts/refactoring/updateImports.ts
