...
// в скриптах есть remove-cache, который позволяет удалить кеш бэбила из проекта. запускается командой node ./scripts/remove-cache.js. Должен удалять после установки библиотек в package.json, но не работает. со скриптом вроде все норм
...
// !!! при генерации отчетов на githubPages разобраться со сторибуком (не подгружаются свг. как вариант решения импортить по прямой ссылке из инета)
